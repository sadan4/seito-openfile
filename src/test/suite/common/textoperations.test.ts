...
suite("Text operation Tests", () => {
  ...
  test("Get word, wrong position", () => {
    ...
    const res = TextOperations.getWordBetweenBounds("hi", pos, configHandler.Configuration.Bound.Value);
    assert.equal(res, "");
  });

  test("Get word, left most position", () => {
    const pos = new vscode.Position(2, 4);
    const res = TextOperations.getWordBetweenBounds('hi "welt"', pos, configHandler.Configuration.Bound.Value);
  ...
  test("Get word, space delimited", () => {
    const pos = new vscode.Position(2, 6);
    const res = TextOperations.getWordBetweenBounds("hey hi/welt", pos, configHandler.Configuration.Bound.Value);
    assert.equal(res, "hi/welt");
  });
  ...
  test('Get word, " bound, missing right "', () => {
    const pos = new vscode.Position(2, 6);
    const res = TextOperations.getWordBetweenBounds(' "hi/welt  puh', pos, configHandler.Configuration.Bound.Value);
    assert.equal(res, "hi/welt");
  });

  test('Get word, " bound, leading .. ', () => {
    const pos = new vscode.Position(2, 16);
    const res = TextOperations.getWordBetweenBounds(
      'ein Text davor "..\\hi\\datei.txt" und dann noch was danach',
      pos,
      configHandler.Configuration.Bound.Value
    );
  ...
  test("Get word, with line column and a colon, cursor before first colon", () => {
    const pos = new vscode.Position(2, 7);
    const res = TextOperations.getWordBetweenBounds(
      "l:/Datas/Test.txt:42:10:content",
      pos,
      configHandler.Configuration.Bound.Value
    );
