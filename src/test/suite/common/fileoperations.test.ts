...
function normalizeSlash(filepath: string): string {
  // Only needed for paths with backslash directory separator (originally developed on Windows).  Paths with '/' are treated as hard-coded as slash.
...
suite("File operation Tests", () => {
  ...
  suiteSetup((done) => {
    openFile = new OpenFileFromText(vscode.window.activeTextEditor, configHandler);
  ...
  test("Github #6: Open with line number", (done) => {
    ...
    openFile
      .openDocument(WS_ROOT + "/Unittests-tmp/test/testcase.txt:2")
      .then((value) => {
        assert.equal(
          vscode.window.activeTextEditor?.document.fileName,
          normalizeSlash(WS_ROOT + "\\Unittests-tmp\\test\\testcase.txt")
        );
  ...
  test("Open with line number and column", (done) => {
    // Remark: WS_ROOT + "/Unittests-tmp" folder must exists before running unit tests.  And the letter case of "Unittests-tmp" folder must be exact match.
    openFile
      .openDocument(WS_ROOT + "/Unittests-tmp/test/testcase.txt:2:5")
      .then((value) => {
        assert.equal(
          vscode.window.activeTextEditor?.document.fileName,
          normalizeSlash(WS_ROOT + "\\Unittests-tmp\\test\\testcase.txt")
  ...
  test("resolvePath, resolve path to home", () => {
    const homeExistentFile =
      os.platform() === "win32"
        ? "~/Desktop/desktop.ini"
        : existsSync("~/.bash_history")
        ? "~/.bash_history"
        : existsSync("~/.zsh_history")
        ? "~/.bashrc"
        : "~/.zshrc";
  ...
  test("resolvePath, resolve path to same folder, fuzzy, extraExtensions", () => {
    configHandler.Configuration.ExtraExtensionsForTypes.Value = [{ name: "js", suffixes: ["ts"] }];
  ...
  test("resolvePath, resolve path to workspace folder", () => {
    configHandler.Configuration.SearchPaths.Value = [
      WS_ROOT + "/",
  ...
  test("resolvePath, resolve path to workspace src folder, without file extension", () => {
    ...
    configHandler.Configuration.SearchSubFoldersOfWorkspaceFolders.Value = ["lib/", "src/"];
  ...
  test("resolvePath, resolve path to searchPaths", () => {
    configHandler.Configuration.SearchPaths.Value = [WS_ROOT + "/"];
  ...
  test("resolvePath, resolve path to 2nd item in searchPaths", () => {
    configHandler.Configuration.SearchPaths.Value = [WS_ROOT + "/", WS_ROOT + "/Unittests-tmp/test/dir1"];
  ...
  test("resolvePath, resolve path with path substitution by leadingPathMapping", () => {
    ...
    configHandler.Configuration.LeadingPathMapping.Value = { "@dir1": "test/dir1" };
  ...
  test("Github #9: resolvePath, resolve path with deletion by leadingPathMapping. Support open paths from git diff which starts with a/ or b/", () => {
    configHandler.Configuration.LeadingPathMapping.Value = { a: "", b: "" };
  ...
  test("resolvePath, resolve path with path substitution, using prefix match with ending '*', by leadingPathMapping.", () => {
    // eslint-disable-next-line @typescript-eslint/naming-convention
    configHandler.Configuration.LeadingPathMapping.Value = { "$*": "*" };
  ...
  test("resolvePath, resolve path with path substitution, using prefix match with ending '*', by leadingPathMapping's deletion, removing path's leading variable", () => {
    // eslint-disable-next-line @typescript-eslint/naming-convention
    configHandler.Configuration.LeadingPathMapping.Value = { "$*": "*", "$*?": "" }; // "$*?" is a special case, same as "$*", but for non-duplicated keys
  ...
  test("resolvePath, resolve path with path substitution, using prefix match with ending '*', by leadingPathMapping, removing '$' for single variable", () => {
    // eslint-disable-next-line @typescript-eslint/naming-convention
    configHandler.Configuration.LeadingPathMapping.Value = { "$*": "*", "$*?": "" }; // "$*?" is a special case, same as "$*", but for non-duplicated keys. Later value in this leadingPathMapping has higher priority.  E.g. such that { "abc": "...", "abc/def": "..." } works better in natural order
    const res = openFile.resolvePath("$Class1", WS_ROOT + "/Unittests-tmp/test/dir1/testcase2.ts");
    assert.equal(res, normalizeSlash(WS_ROOT + "\\Unittests-tmp\\src\\Class1.ts"));
  });
  ...
  test("Gitlab issue 23, open file which has no suffix at all, added path to search list", () => {
    configHandler.Configuration.SearchPaths.Value = [WS_ROOT + "/", WS_ROOT + "/Unittests-tmp/src"];
  ...
  test("Gitlab issue 23, open file which has no suffix at all, added path to search list and subfolders", () => {
    configHandler.Configuration.SearchPaths.Value = [WS_ROOT + "/", WS_ROOT + "/Unittests-tmp"];
    configHandler.Configuration.SearchSubFoldersOfWorkspaceFolders.Value = ["lib/", "src/"];
    const res = openFile.resolvePath("Class12", WS_ROOT + "/Unittests-tmp/test/dir1/testcase2.ts");
    assert.equal(res, normalizeSlash(WS_ROOT + "\\Unittests-tmp\\src\\Class12"));
  });
  ...
  test("Issue 30: Open in file explorer if path string is just a path", () => {
    configHandler.Configuration.SearchPaths.Value = [WS_ROOT + "/"];
    configHandler.Configuration.PreferOpenFile.Value = false;
